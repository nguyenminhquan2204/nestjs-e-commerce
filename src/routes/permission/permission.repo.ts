...
export class PermissionRepo {
   ...
   async list(pagination: GetPermissionsQueryType): Promise<GetPermissionsResType> {
      ...
      const [totalItems, data] = await Promise.all([
         ...
         this.prismaService.permission.findMany({ where: { deletedAt: null }, skip, take })
      ])
      return {
         data,
         totalItems,
   ...
   update({ updatedById, id, data }: { updatedById: number | null, id: number, data: UpdatePermissionBodyType }): Promise<PermissionType> {
   ...
   delete(id: number, userId: number, isHard?: boolean): Promise<PermissionType> {
      return isHard ? this.prismaService.permission.delete({ where: { id }}) : this.prismaService.permission.update({ where: { id, deletedAt: null}, data: { deletedAt: new Date(), deletedById: userId }})
   }
}